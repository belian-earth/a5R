...
1. **`a5_cell_to_arrow()`**: converts the hi/lo double fields to 8-byte
   little-endian raw vectors (one per cell), creates an Arrow
...
2. **`a5_cell_from_arrow()`**: does the reverse — `View(fixed_size_binary(8))`
   on the `uint64` array to get the raw bytes, then converts each 8-byte
   vector to hi/lo doubles via bit-shift.
