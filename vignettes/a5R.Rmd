...
Multi-part inputs (a `POLYGON` with holes, a `MULTIPOLYGON`, or an
`sfc` of several polygons) are rejected by default. Opt in with
`handle_multigeom = TRUE` and each part is converted independently and
the results are unioned. Only outer rings contribute meaningfully:
holes are not subtracted, so cells inside any hole remain in the
output.

```{r, eval = requireNamespace("sf", quietly = TRUE)}
library(sf)
demo(nc, ask = FALSE, echo = FALSE)
nca5 <- a5_polygon_to_cells(nc, resolution = 9, handle_multigeom = TRUE) |>
  a5_uncompact(9)
...
follow the same rule as `a5_polygon_to_cells()`: rejected by default,
allowed via `handle_multigeom = TRUE`, with per-feature outputs
concatenated and deduplicated first-seen.
