...
| `a5_polygon_to_cells()` | Heavy (per-part flood-fill, hole subtraction) | High |
| `a5_linestring_to_cells()` | Heavy (per-feature tracing) | High |
| `a5_spherical_cap()` | Heavy (per-cell search) | High |
| `a5_grid_disk()` | Heavy (per-cell traversal) | High |
...
| `a5_cell_to_children()` | Light (bit ops per child) | Low |
| `a5_cell_to_parent()` | Light (bit ops + hex) | Low |
| `a5_get_resolution()` | Light (bit ops) | Low |
| `a5_is_valid()` | Light (hex parse) | Low |

For the one-to-many functions (`a5_grid_disk()`, `a5_spherical_cap()`,
`a5_cell_to_children()`) the per-cell work is parallelised; assembling the
result, especially the `simplify = FALSE` list, is sequential, so the list
form gains less than the flat form.

Scalar and bulk operations (`a5_compact()`, `a5_cell_area()`,
`a5_cell_child()`, `a5_cell_children_range()`, etc.) are unaffected: they are
already fast or delegate to algorithms that don't parallelise element-wise.
...
`elapsed` (wall-clock) time decreases: that is the parallelism at work.
...
changing the count mid-session is fine (and cheap) but not free: ideally,
